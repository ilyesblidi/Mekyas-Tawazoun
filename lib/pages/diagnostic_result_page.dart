...
class DiagnosticResultPage extends StatelessWidget {
  ...
  static Color scoreColor(double avg) {
    if (avg < 1) return Colors.red;
    if (avg < 2) return Colors.orange;
    if (avg < 3) return Colors.amber;
    if (avg < 4) return Colors.lightGreen;
    return Colors.green;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final percentages = fields.map((f) => double.tryParse(result[f]?['percentage'] ?? '0') ?? 0.0).toList();
    final avgs = fields.map((f) {
      final answers = Map<String, dynamic>.from(result[f]?['answers'] ?? {});
      if (answers.isEmpty) return 0.0;
      return answers.values.fold(0, (a, b) => a + (b as int)) / answers.length;
    }).toList();

    // Calculate general percentage
    final generalPercent = percentages.isNotEmpty
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          elevation: 2,
          title: const Text('نتائج التشخيص', style: TextStyle(fontFamily: 'Cairo', color: Color(0xFF1A6F8E))),
          centerTitle: true,
          actions: [
            IconButton(
                ...
                child: Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: RadarChart(
                      ...
                      featuresTextStyle: const TextStyle(fontFamily: 'Cairo', fontSize: 13, color: Color(0xFF1A6F8E)),
                      outlineColor: Colors.grey[300]!,
                      graphColors: [const Color(0xFF1A6F8E)],
                      // Remove tickColor if not supported by your package version
                    ),
                  )
                ),
              ),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(fieldNames[fields[i]]!, style: const TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1A6F8E))),
                              const SizedBox(height: 4),
                              Row(
                                children: [
                                  Text('المعدل: ${avg.toStringAsFixed(1)} / 5', style: const TextStyle(fontFamily: 'Cairo', fontSize: 13)),
                                  const SizedBox(width: 12),
                                  Text(interpretScore(avg), style: TextStyle(fontFamily: 'Cairo', fontSize: 13, color: scoreColor(avg))),
                                ],
                              ),
                              const SizedBox(height: 8),
                              LinearProgressIndicator(
                                ...
                                valueColor: AlwaysStoppedAnimation<Color>(scoreColor(avg)),
                              ),
                            ],
                          ),
                        ),
                ...
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      label: const Text('إنشاء خطة عمل', style: TextStyle(color: Colors.white)),
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      ...
                      label: const Text('إعادة التشخيص', style: TextStyle(color: Color(0xFF1A6F8E))),
                      onPressed: onRetake,
                    ),
                  ),
                ],
