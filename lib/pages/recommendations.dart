            ...
            class _RecomandationsPageState extends State<RecomandationsPage> {
              ...
              Future<void> _fetchAndUpdateFromFirestore() async {
                ...
                final doc = await FirebaseFirestore.instance
                    ...
                    .get();
                if (doc.exists) {
                  final recs = doc.data()?['recommendations'] as Map<String, dynamic>? ?? {};
                  if (recs.isNotEmpty) {
              ...
              void _initializeControllers(Map<String, dynamic> data) {
                for (final field in data.keys) {
                  ...
                  for (final question in answers.keys) {
                    final recText = recommendations[field] != null
                        ? (recommendations[field][question] ?? '')
                        : '';
                    recommendationControllers[field]![question] ??= TextEditingController(text: recText);
                  }
                }
              }
              ...
              Future<void> saveRecommendations() async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('تم حفظ التوصيات بنجاح')),
                );
              }
                            ...
                            builder: (context, snapshot) {
                              ...
                              return ListView.builder(
                                ...
                                itemBuilder: (context, idx) {
                                  ...
                                  final percent =
                                      double.tryParse(fieldData['percentage']?.toString() ?? '0') ?? 0;
                                            ...
                                            child: ListView.separated(
                                              ...
                                              itemBuilder: (context, qIdx) {
                                                ...
                                                recommendationControllers[fieldName]![qEntry.key] ??= TextEditingController(
                                                  text: recommendations[fieldName] != null
                                                      ? (recommendations[fieldName][qEntry.key] ?? '')
                                                      : '',
                                                );
